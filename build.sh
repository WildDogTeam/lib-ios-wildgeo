...
XCODE_PROJECT="${DIR}/WilddogGeo.xcodeproj"
...
lipo \
  -create \
    ${OUTPUT_DIR}/Products/Release-iphoneos/libWilddogGeo.a \
    ${OUTPUT_DIR}/Products/Release-iphonesimulator/libWilddogGeo.a \
  -output ${OUTPUT_DIR}/Products/Release-combined/WilddogGeo.framework/Versions/A/WilddogGeo
echo " done."

echo -n "===> Checking how the final binary looks ..."
EXPECTEDCOUNT=6
ARCHCOUNT=$(file ${OUTPUT_DIR}/Products/Release-combined/WilddogGeo.framework/Versions/A/WilddogGeo | wc -l)
if [[ $ARCHCOUNT -ne $EXPECTEDCOUNT ]]; then
  echo " bad."
  file ${OUTPUT_DIR}/Products/Release-combined/WilddogGeo.framework/Versions/A/WilddogGeo
...
zip -ry ../../WilddogGeo.framework.zip WilddogGeo.framework
popd

ls -l target/WilddogGeo.framework.zip
