...
//                        format:@"Not a valid geo location: [%f,%f]", location.latitude, location.longitude];
        ...
        for (NSUInteger i = 0; i < precision; i++) {
            ...
            buffer[i] = [GWBase32Utils valueToBase32Character:hashVal];
        }
...
{
    return [GWGeoHash newWithLocation:location precision:GF_DEFAULT_PRECISION];
}

+ (GWGeoHash *)newWithLocation:(CLLocationCoordinate2D)location precision:(NSUInteger)precision
{
    return [[GWGeoHash alloc] initWithLocation:location precision:precision];
}

+ (GWGeoHash *)newWithString:(NSString *)string
{
    return [[GWGeoHash alloc] initWithString:string];
}

- (id)initWithString:(NSString *)hashValue
{
    if ([GWGeoHash isValidGeoHash:hashValue]) {
...
+ (BOOL)isValidGeoHash:(NSString *)hash
{
    ...
    dispatch_once(&onceToken, ^{
        base32Set = [NSCharacterSet characterSetWithCharactersInString:[GWBase32Utils base32Characters]];
    });
    if (hash.length == 0) {
        return NO;
    }
...
{
    return [NSString stringWithFormat:@"GWGeoHash: %@", self.geoHashValue];
}
