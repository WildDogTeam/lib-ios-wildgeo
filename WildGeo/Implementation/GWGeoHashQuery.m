...
+ (GWGeoHashQuery *)newWithStartValue:(NSString *)startValue endValue:(NSString *)endValue
{
    return [[GWGeoHashQuery alloc] initWithStartValue:startValue endValue:endValue];
}
...
{
    double degrees = [GWGeoHashQuery meters:resolution toLongitudeDegreesAtLatitude:latitude];
...
+ (NSUInteger)bitsForBoundingBoxAtLocation:(CLLocationCoordinate2D)location withSize:(double)size
{
    ...
    NSUInteger bitsLatitude = MAX(0, (int)floor([GWGeoHashQuery bitsForLatitudeWithResolution:size]))*2;
    double bitsNorth = [GWGeoHashQuery bitsForLongitudeWithResolution:size atLatitude:latitudeNorth];
    double bitsSouth = [GWGeoHashQuery bitsForLongitudeWithResolution:size atLatitude:latitudeSouth];
...
+ (GWGeoHashQuery *)geoHashQueryWithGeoHash:(GWGeoHash *)geohash bits:(NSUInteger)bits
{
    NSString *hash = geohash.geoHashValue;
    NSUInteger precision = ((bits-1)/BITS_PER_GEOHASH_CHAR)+1;
    if (hash.length < precision) {
        return [GWGeoHashQuery newWithStartValue:hash endValue:[NSString stringWithFormat:@"%@~", hash]];
    }
    hash = [hash substringToIndex:precision];
    NSString *base = [hash substringToIndex:hash.length-1];
    NSUInteger lastValue = [GWBase32Utils base32CharacterToValue:(char)[hash characterAtIndex:hash.length-1]];
    ...
    NSString *startHash = [NSString stringWithFormat:@"%@%c", base, [GWBase32Utils valueToBase32Character:startValue]];
    NSString *endHash;
    if (endValue > 31) {
        endHash = [NSString stringWithFormat:@"%@~", base];
    } else {
        endHash = [NSString stringWithFormat:@"%@%c", base, [GWBase32Utils valueToBase32Character:endValue]];
    }
    return [GWGeoHashQuery newWithStartValue:startHash endValue:endHash];
}
...
+ (NSSet *)joinQueries:(NSSet *)set
{
    ...
    BOOL didJoin;
    do {
        GWGeoHashQuery *query1 = nil;
        GWGeoHashQuery *query2 = nil;
        for (GWGeoHashQuery *query in queries) {
            for (GWGeoHashQuery *other in queries) {
...
{
    NSUInteger bits = [GWGeoHashQuery bitsForRegion:region];
    ...
    void (^addQuery)(CLLocationDegrees, CLLocationDegrees) = ^(CLLocationDegrees lat, CLLocationDegrees lng) {
        GWGeoHash *geoHash = [GWGeoHash newWithLocation:CLLocationCoordinate2DMake(lat, lng)
                                              precision:geoHashPrecision];
        [queries addObject:[GWGeoHashQuery geoHashQueryWithGeoHash:geoHash bits:bits]];
    };
    ...
    CLLocationDegrees longitudeWest = [GWGeoHashQuery
                                       wrapLongitude:(region.center.longitude - region.span.longitudeDelta/2)];
    CLLocationDegrees longitudeEast = [GWGeoHashQuery
                                       wrapLongitude:(region.center.longitude + region.span.longitudeDelta/2)];
    ...
    return [GWGeoHashQuery joinQueries:queries];
}
...
+ (NSSet *)queriesForLocation:(CLLocationCoordinate2D)center radius:(double)radius
{
    ...
    CLLocationDegrees longitudeDeltaNorth = [GWGeoHashQuery meters:radius toLongitudeDegreesAtLatitude:latitudeNorth];
    CLLocationDegrees longitudeDeltaSouth = [GWGeoHashQuery meters:radius toLongitudeDegreesAtLatitude:latitudeSouth];
    ...
    return [GWGeoHashQuery queriesForRegion:region];
}

- (BOOL)isPrefixTo:(GWGeoHashQuery *)other
{
...
- (BOOL)isSuperQueryOf:(GWGeoHashQuery *)other
{
...
- (GWGeoHashQuery *)joinWith:(GWGeoHashQuery *)other
{
    if ([self isPrefixTo:other]) {
        return [[GWGeoHashQuery alloc] initWithStartValue:self.startValue endValue:other.endValue];
    } else if ([other isPrefixTo:self]) {
        return [[GWGeoHashQuery alloc] initWithStartValue:other.startValue endValue:self.endValue];
...
- (BOOL)isEqual:(id)other {
    if (other == self)
        return YES;
    if (![other isKindOfClass:[GWGeoHashQuery class]])
        return NO;
...
{
    return [NSString stringWithFormat:@"GWGeoHashQuery: %@-%@", self.startValue, self.endValue];
}
